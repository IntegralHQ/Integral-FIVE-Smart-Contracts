...
describe('IntegralDelay.setTransferGasCost', () => {
  ...
  it('can be changed', async () => {
    ...
    await expect(delay.connect(other.address).setTransferGasCost(token0.address, 50000)).to.be.revertedWith(
      'ID_FORBIDDEN'
    )

    await expect(delay.setTransferGasCost(token0.address, 50000))
