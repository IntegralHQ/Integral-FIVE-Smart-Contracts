...
describe('IntegralTimeRelease.initOption1Allocations', () => {
  const loadFixture = setupFixtureLoader()

  it('non owner', async () => {
    const { timeRelease, other } = await loadFixture(timeReleaseFixture)
    await expect(timeRelease.connect(other).initOption1Allocations([], [], overrides)).to.be.revertedWith(
      'TR_FORBIDDEN'
    )
  })
  ...
  it('different inputs lengths ', async () => {
    ...
    await expect(
      timeRelease.initOption1Allocations([wallet.address, other.address], [1], overrides)
  ...
  it('after skim', async () => {
    ...
    await expect(timeRelease.initOption1Allocations([], [], overrides)).to.be.revertedWith('TR_STOP_ALREADY_SET')
  })
  ...
  it('sets allocations and votes', async () => {
    ...
    const tx = await timeRelease.initOption1Allocations(wallets, amounts, overrides)
    const blockNumber = (await tx.wait()).blockNumber

    for (let i = 0; i < wallets.length; i++) {
      expect(await timeRelease.getOption1Allocation(wallets[i])).to.eq(amounts[i])
      expect(await timeRelease.getPriorVotes(wallets[i], blockNumber)).to.eq(amounts[i])
    }
    expect(await timeRelease.option1TotalAllocations()).to.eq(totalAllocation)
  })
  ...
  it('allocation exists', async () => {
    ...
    await timeRelease.initOption1Allocations([wallet.address], [expandTo18Decimals(1)])
    await expect(timeRelease.initOption1Allocations([wallet.address], [expandTo18Decimals(2)])).to.be.revertedWith(
  ...
  it('allocation zero', async () => {
    ...
    await expect(timeRelease.initOption1Allocations([wallet.address], [BigNumber.from(0)])).to.be.revertedWith(
      'TR_ALLOCATION_ZERO'
    )
  })
  ...
  it('after claim', async () => {
    ...
    await expect(
      timeRelease.initOption1Allocations([another.address], [expandTo18Decimals(1)], overrides)
    ).to.be.revertedWith('TR_INSUFFICIENT_BALANCE')
    await token.mint(timeRelease.address, expandTo18Decimals(1))
    await timeRelease.initOption1Allocations([another.address], [expandTo18Decimals(1)], overrides)
    expect(await timeRelease.getOption1Allocation(another.address)).to.eq(expandTo18Decimals(1))
  })

  // skip is intentional
  it.skip('plenty of inputs', async () => {
    const { timeRelease, token } = await loadFixture(timeReleaseFixture)
    const wallets = [...new Array(2004)].map(() => Wallet.createRandom().address)
    const amounts = [...new Array(2004)].map(() => utils.parseUnits(Math.floor(Math.random() * 10 + 1).toString()))
    const totalAllocation = amounts.reduce((acc: BigNumber, curr: BigNumber) => acc.add(curr), BigNumber.from(0))
    await token.mint(timeRelease.address, totalAllocation)

    const maxSliceLength = 367
    for (let i = 0; i < 6; i++) {
      await timeRelease.initOption1Allocations(
    ...
    expect(await timeRelease.getOption1Allocation(wallets[0])).to.eq(amounts[0])
    expect(await timeRelease.getOption1Allocation(wallets[10])).to.eq(amounts[10])
    expect(await timeRelease.getOption1Allocation(wallets[500])).to.eq(amounts[500])
    expect(await timeRelease.getOption1Allocation(wallets[1000])).to.eq(amounts[1000])
    expect(await timeRelease.getOption1Allocation(wallets[1500])).to.eq(amounts[1500])
    expect(await timeRelease.getOption1Allocation(wallets[1999])).to.eq(amounts[1999])
    expect(await timeRelease.option1TotalAllocations()).to.eq(totalAllocation)
  }).timeout(60_000)
})
