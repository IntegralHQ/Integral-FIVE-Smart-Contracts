...
describe('IntegralPair.setToken0RelativeLimit', () => {
  ...
  it('fails if limit is exceeded', async () => {
    ...
    await expect(factory.setToken0RelativeLimit(token0.address, token1.address, expandTo18Decimals(0.5), overrides))
  ...
  it('allows for swaps that repair the limit', async () => {
    ...
    await addLiquidity(expandTo18Decimals(100), expandTo18Decimals(100))
    await factory.setToken0RelativeLimit(token0.address, token1.address, expandTo18Decimals(0.3), overrides)
