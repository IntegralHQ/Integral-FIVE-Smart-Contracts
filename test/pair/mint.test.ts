...
describe('IntegralPair.mint', () => {
  ...
  it('applies the fee and correctly calculates references', async () => {
    const { pair, oracle, token0, addLiquidity, SWAP_FEE_N, MINT_FEE, getState, PRECISION, wallet } = await loadFixture(
    ...
    expect(after.references[0]).to.equal(expandTo18Decimals(500 + amount0))
    expect(after.references[1]).to.equal(expandTo18Decimals(500).add(expandTo18Decimals(amount1)))
  ...
  it('reverts if to is zero', async () => {
    ...
    await expect(pair.mint(constants.AddressZero)).to.revertedWith('IP_ADDRESS_ZERO')
  })
})
