...
describe('IntegralPair.setTrader', () => {
  ...
  it('allows anyone to trade when set to -1', async () => {
    ...
    await expect(factory.setTrader(token0.address, token1.address, `0x${'f'.repeat(40)}`, overrides)).to.emit(
      pair,
      'SetTrader'
    )
    // .withArgs(`0x${'f'.repeat(40)}`) doesn't work as arg has both 'f' and 'F' characters

    await expect(pair.connect(wallet).mint(other.address, overrides)).not.to.be.revertedWith('IP_UNAUTHORIZED_TRADER')
    await expect(pair.connect(wallet).burn(other.address, overrides)).not.to.be.revertedWith('IP_UNAUTHORIZED_TRADER')
    await expect(pair.connect(wallet).swap(0, 1, other.address, overrides)).not.to.be.revertedWith(
      'IP_UNAUTHORIZED_TRADER'
    )

    await expect(pair.connect(other).mint(other.address, overrides)).not.to.be.revertedWith('IP_UNAUTHORIZED_TRADER')
    await expect(pair.connect(other).burn(other.address, overrides)).not.to.be.revertedWith('IP_UNAUTHORIZED_TRADER')
    await expect(pair.connect(other).swap(0, 1, other.address, overrides)).not.to.be.revertedWith(
      'IP_UNAUTHORIZED_TRADER'
    )
  })
