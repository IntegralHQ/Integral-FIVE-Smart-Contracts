...
import { integralAndUniswapFixture } from './shared/fixtures/integralAndUniswapFixture'
...
describe('integrationV2', () => {
  ...
  it('makes different transactions on pairs', async () => {
    const {
      ...
      uniswapPair23,
      swapOnUniswapPair,
    } = await loadFixture(integralAndUniswapFixture)
    ...
    async function withdraw(tokenX: IERC20, tokenY: IERC20, pair: IntegralPair, liquidity: number) {
