...
export async function timeReleaseFixture([wallet]: Wallet[]) {
  ...
  const timeRelease = await new IntegralTimeReleaseTest__factory(wallet).deploy(
    token.address,
    0,
    100,
    0,
  ...
  async function setupTimeRelease(option1Timeframe: number[], option2Timeframe: number[]) {
    await timeRelease.setOption1Timeframe(option1Timeframe[0], option1Timeframe[1], overrides)
    await timeRelease.setOption2Timeframe(option2Timeframe[0], option2Timeframe[1], overrides)
    ...
    return async function setAllocations(_wallet: Wallet, option1Raw: number, option2Raw: number) {
      ...
      option1Raw && (await timeRelease.initOption1Allocations([_wallet.address], [option1Allocation], overrides))
      option2Raw && (await timeRelease.initOption2Allocations([_wallet.address], [option2Allocation], overrides))
    }
  }
